...
def top_locations_mappable(violations_df : pd.DataFrame, threshold=1000) -> pd.DataFrame:
    ...
    top_loc_deduped_df = top_loc_df.drop_duplicates(subset='location')
    top_loc_deduped_df = top_loc_deduped_df.rename(columns={'amount_x': 'amount'})
    return top_loc_deduped_df
...
if __name__ == '__main__':
    ...
    tickets = tickets_in_top_locations(df)
